    ...
    <ul v-if="open" class="mt-2 space-y-2">
      ...
      >
        <div class="flex flex-wrap items-start gap-2">
          <p class="min-w-0 flex-1">{{ noticeMessage(warning) }}</p>
          <button
      ...
      <!-- General notices from the backend (OCR quality hints, docling
           fallback notes, …) — rendered as informational items, separate from
           the structured validation warnings above. Each carries a code the
           catalogs translate, with the backend's English text as fallback. -->
      <li
